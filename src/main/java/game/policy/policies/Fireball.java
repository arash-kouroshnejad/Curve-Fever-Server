...
public class Fireball extends PowerupPolicy {

    Timer timer = new Timer();
    DynamicElement affected;

    public Fireball(GameLogic logic, PolicyReference policyReference) {
        super(logic, policyReference);
    }

    @Override
    public void enforce(DynamicElement element1, DynamicElement element2) {
        var element = element1.getType().equals("Fireball") ? element1 : element2;
    ...
    private void enableEffect() {
        ...
        if (affected == policyReference.getPlayer1Character()) {
            ...
            policyReference.setPlayer1State(Rewards.Fireball);
            policyReference.setPlayer1Effect(this);
        }
        else {
            last = policyReference.getPlayer2Effect();
            policyReference.setPlayer2State(Rewards.Fireball);
    ...
    public boolean isEnforceable(DynamicElement element1, DynamicElement element2) {
        return "Fireball".equals(element1.getType()) || "Fireball".equals(element2.getType());
    }
}
